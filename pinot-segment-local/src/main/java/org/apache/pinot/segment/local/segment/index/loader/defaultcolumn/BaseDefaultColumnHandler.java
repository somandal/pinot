...
public abstract class BaseDefaultColumnHandler implements DefaultColumnHandler {
  ...
  Map<String, DefaultColumnAction> computeDefaultColumnActionMap() {
    ...
    for (String column : columnsInSchema) {
      ...
      if (columnMetadata != null) {
        ...
        // Check that dictionary with compression isn't enabled / disabled on an existing column.
        // TODO: Add support for reloading segments when dictionary with compression is enabled or disabled
        if (columnMetadata.hasDictionaryWithCompression() != _indexLoadingConfig
            .getDictionaryCompressionForwardIndexCompressionType().containsKey(column)) {
          String failureMessage = "Dictionary with compression flag: " + columnMetadata.hasDictionaryWithCompression()
              + " for column: " + column + " does not match dictionary with compression flag: "
              + _indexLoadingConfig.getDictionaryCompressionForwardIndexCompressionType().containsKey(column)
  ...
  private void createDefaultValueColumnV1Indices(String column)
    ...
    SegmentColumnarIndexCreator.addColumnMetadataInfo(_segmentProperties, column, columnIndexCreationInfo, totalDocs,
        fieldSpec, true/*hasDictionary*/, dictionaryElementSize, false);
  }
