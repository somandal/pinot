...
/**
 * Class to write out fixed length bytes in the form of dictionary IDs into a
 * single column. Client responsible to ensure that they call the correct
 * set method that matches the sizeOfEntry. Avoiding checks here, as they
 * can be expensive when called for each row.
 *
 ...
 */
@NotThreadSafe
public class FixedByteDictionaryChunkSVForwardIndexWriter extends BaseChunkSVForwardIndexWriter {
   ...
   * @param writerVersion version of File
   * @throws IOException if the file isn't found or can't be mapped
   */
  public FixedByteDictionaryChunkSVForwardIndexWriter(File file, ChunkCompressionType compressionType, int totalDocs,
    ...
    super(file, compressionType, totalDocs, normalizeDocsPerChunk(writerVersion, numDocsPerChunk),
        ((long) sizeOfEntry * normalizeDocsPerChunk(writerVersion, numDocsPerChunk)), sizeOfEntry, writerVersion,
        true);
    _chunkDataOffset = 0;
  }

  public void putDictId(int value) {
